...
namespace Ambient\Paytabs\Model;
...
class PaymentMethod extends \Magento\Payment\Model\Method\AbstractMethod {
    ...
    public function getPostHTML($order, $storeId = null) {
        ...
        $secretsign = $this->getConfigData('secretsign');


        $returnUrl = self::getReturnUrl();
	      $cancelFailUrl= self::getCancelFailUrl();
        ...
        $locale = substr($locale, 0, 2); //Get Code

        if (strtolower($locale) != 'ar')
            $locale = 'en';

        $html = "<form name='paytabsexp-form' id='paytabsexp-form' method='POST'>
					<div class='buttons'>
						<div class=''>
							<div style='margin-top:20px; margin-bottom-12px; height:565px;' class='PT_express_checkout'></div>
						</div>
					</div>
				</form>";
    ...
    public function getCancelFailUrl($storeId = null){
    	return $this->_getUrl('ambient/ipn/cancel',$storeId, false);
    }
    /**
     * Get cancel URL.
     *
